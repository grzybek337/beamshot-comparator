...
* **Usage:** Intended for comparing a small amount of lights (e.g., Review Item vs Competitor) within a single article.
...
* `script-main.ps1`: Generates configuration for the Main Comparator (prompts for specifications, autofills them and calculates cd/lm).
...
This is the main step. The tool relies on a list of data (names, specs, and image URLs) located inside the HTML file. You can generate this list automatically using the included scripts or write it manually.
...
4.  Right-click the script (`script-main.ps1` or `script-compact-reviews.ps1`) and select **Edit**.
...
8.  Copy the block and paste in the correct place in the main comparator script.
...
3.  Copy and paste the template below to add a new entry.
